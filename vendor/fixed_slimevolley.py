"""
Modified Port of Neural Slime Volleyball to Python Gym Environment

Original version by David Ha (2020):
...
This file is a modified version of the original SlimeVolleyGym implementation.
It has been adapted to work with the current project structure and requirements.

Modifications:
1. Added Pygame support for rendering, alongside the original rendering methods.
2. Introduced new display methods (display_pygame) for various game elements.
3. Modified the Game class to include Pygame-specific rendering.
4. Adjusted window dimensions and scaling factors for better visualization.
5. Added compatibility layers to ensure both original and Pygame rendering work.
6. Removed or commented out unused code related to the original rendering system.

Note: This version is intended to be used as a local replacement for the
original SlimeVolleyGym library. It is placed in the project directory
to ensure version control and project-specific customization.

Usage:
- Import SlimeVolleyEnv from this file instead of the original library.
- Ensure this file is in the correct project directory (e.g., vendor/fixed_slimevolley.py).

Dependencies: Numpy, Gym, OpenCV (cv2), Pygame
...
WINDOW_WIDTH = 1200  # Increase window size
WINDOW_HEIGHT = 500
FACTOR = WINDOW_WIDTH / REF_W  # This will automatically scale all elements
...
class Particle:
  ...
  def __init__(self, x, y, vx, vy, r, c):
    ...
    self.r = r
    self.c = c
  def display_pygame(self, screen):
    x = int(FACTOR * (self.x + REF_W/2))
    y = int(WINDOW_HEIGHT - FACTOR * self.y)  # Changed this line
    r = int(FACTOR * self.r)
...
class Wall:
  ...
  def display_pygame(self, screen):
        ...
        pygame.draw.rect(screen, color, (0, WINDOW_HEIGHT - int(FACTOR * self.h), WINDOW_WIDTH, int(FACTOR * self.h)))
  def display(self, canvas):
...
class Agent:
  ...
  def display_pygame(self, screen):
    ...
    # Draw the eye
    eye_x = x + int(0.6 * r * self.dir)
    eye_y = y - int(0.6 * r)
    pygame.draw.circle(screen, (255, 255, 255), (eye_x, eye_y), int(r * 0.3))
    pygame.draw.circle(screen, (0, 0, 0), (eye_x, eye_y), int(r * 0.1))
...
class Game:
  ...
  def display_pygame(self, screen):
    ...
    # Draw ground
    ground_height = int(FACTOR * self.ground.h)
    pygame.draw.rect(screen, self.ground.c, (0, WINDOW_HEIGHT - ground_height, WINDOW_WIDTH, ground_height))
    
    # Draw fence
    fence_x = WINDOW_WIDTH // 2
    fence_height = int(FACTOR * REF_WALL_HEIGHT)
    pygame.draw.rect(screen, FENCE_COLOR, (fence_x - 5, WINDOW_HEIGHT - fence_height, 10, fence_height))
    
    # Draw left agent
    left_x = int(FACTOR * (self.agent_left.x + REF_W/2))
    left_y = int(WINDOW_HEIGHT - FACTOR * self.agent_left.y)
    left_r = int(FACTOR * self.agent_left.r)
    pygame.draw.circle(screen, self.agent_left.c, (left_x, left_y), left_r)
    
    # Draw right agent
    right_x = int(FACTOR * (self.agent_right.x + REF_W/2))
    right_y = int(WINDOW_HEIGHT - FACTOR * self.agent_right.y)
    right_r = int(FACTOR * self.agent_right.r)
    pygame.draw.circle(screen, self.agent_right.c, (right_x, right_y), right_r)
    
    # Draw ball
    ball_x = int(FACTOR * (self.ball.x + REF_W/2))
    ball_y = int(WINDOW_HEIGHT - FACTOR * self.ball.y)
    ball_r = int(FACTOR * self.ball.r)
    pygame.draw.circle(screen, self.ball.c, (ball_x, ball_y), ball_r)
...
class SlimeVolleyEnv(gym.Env):
  ...
  def step(self, action, otherAction=None):
    ...
    info = {
      'ale.lives': self.game.agent_right.life,
      'ale.otherLives': self.game.agent_left.life,
      'otherObs': otherObs,
  ...
  def render(self, mode='human'):
    if self.viewer is None:
        ...
        pygame.display.set_caption("Slime Volley")
        self.viewer = True
    
    self.game.display_pygame(self.screen)
